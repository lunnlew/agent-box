...
if ! pgrep -f "x11vnc.*${OPENCLAW_MANAGER_VNC_PORT}" > /dev/null; then
    ...
    x11vnc -display WAIT:${DISPLAY_NUM} -forever -shared -rfbport ${OPENCLAW_MANAGER_VNC_PORT} -nopw -bg -noxdamage -noxfixes
    sleep 2
    if ! pgrep -f "x11vnc.*${OPENCLAW_MANAGER_VNC_PORT}" > /dev/null; then
...
if command -v websockify &> /dev/null; then
    websockify --web=/usr/share/novnc/ ${OPENCLAW_MANAGER_NOVNC_PORT} localhost:${OPENCLAW_MANAGER_VNC_PORT} &
elif [ -f /usr/share/novnc/websockify/websockify.py ]; then
    python3 /usr/share/novnc/websockify/websockify.py --web=/usr/share/novnc/ ${OPENCLAW_MANAGER_NOVNC_PORT} localhost:${OPENCLAW_MANAGER_VNC_PORT} &
else
