...
if command -v websockify &> /dev/null; then
    websockify --web="${NOVNC_WEB_DIR}" ${SKILLS_MANAGER_NOVNC_PORT} localhost:${SKILLS_MANAGER_VNC_PORT} &
else
...
if ! pgrep -f "websockify.*${SKILLS_MANAGER_NOVNC_PORT}" > /dev/null; then
    echo "ERROR: Failed to start websockify on port ${SKILLS_MANAGER_NOVNC_PORT}"
    exit 1
fi
