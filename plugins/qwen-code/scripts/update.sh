#!/bin/bash
set +e

# 导入共享函数库
source /opt/lib.sh 2>/dev/null || {
  log_info() { echo "[INFO] $1"; }
  log_success() { echo "[SUCCESS] $1"; }
  log_warning() { echo "[WARNING] $1"; }
  log_error() { echo "[ERROR] $1"; }
}

PKG_NAME="@qwen-code/qwen-code"
PKG_PATH="$(npm root -g 2>/dev/null)/@qwen-code/qwen-code"

log_info "Updating Qwen Code..."

# 备份用户配置
if [ -d ~/.qwen-code ]; then
  cp -r ~/.qwen-code ~/.qwen-code.bak.$(date +%Y%m%d%H%M%S) 2>/dev/null || true
fi

# 更新
npm update -g "$PKG_NAME" || {
  log_warning "Update failed, reinstalling..."
  npm install -g "$PKG_NAME"@latest
}

# 验证
if [ ! -f "$PKG_PATH/package.json" ]; then
  log_error "Update verification failed"
  if [ -d ~/.qwen-code.bak.* ]; then
    mv ~/.qwen-code.bak.* ~/.qwen-code 2>/dev/null || true
  fi
  exit 1
fi

qwen --version
log_success "Qwen Code updated successfully"

# 清理备份
rm -rf ~/.qwen-code.bak.* 2>/dev/null || true